...
- **THEN** a `lsa_security_events` row is created with status `pending`, the source IP, pattern type, and raw log excerpt
...
The `lsa_security_events` table SHALL contain: `id`, `ip_address`, `pattern_type`, `raw_excerpt` (text), `agent_summary` (text, nullable), `confidence` (float, nullable), `outcome` (enum: pending/blocked/alerted/ignored), `created_at`, `updated_at`.
